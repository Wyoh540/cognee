...
export default function UsersPanel() {
  ...
  const handleToggle = async (userId: string, field: "is_superuser" | "is_active", currentValue: boolean) => {
      ...
      <Card style={{ flex: 1, overflow: "auto" }}>
        {loading ? (
          ...
          <div style={{ textAlign: "center", padding: "2rem 0" }}>
            ...
            <Button onClick={loadUsers} variant="subtle" size="xs"
              styles={{ root: { color: C.accent, "&:hover": { background: "rgba(188,155,255,0.1)" } } }}>
              Retry
            </Button>
          </div>
        ) : users.length === 0 ? (
    ...
    <>
      <Card style={{ flex: 1, overflow: "auto" }}>
        {loading ? (
          ...
          <div><Text size="sm" style={{ color: C.danger, marginBottom: 12 }}>{error}</Text></div>
        ) : users.length === 0 ? (
          <div><Text size="sm">No users found.</Text></div>
        ) : (
          <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
            {users.map((u) => (
              <div key={u.id} style={{ display: "grid", gridTemplateColumns: COLUMNS, gap: 16, alignItems: "center", padding: "10px 0" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div style={{ width: 32, height: 32, borderRadius: "50%", background: "rgba(188,155,255,0.15)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <PersonIcon size={18} />
                  </div>
                  <Text size="sm" style={{ color: C.textPrimary }}>{u.email}</Text>
                </div>
                <div style={{ display: "flex", justifyContent: "center" }}>
                  {u.is_superuser ? (
                    <span style={{ background: C.accentBg, color: C.accent, border: C.accentBorder, padding: "2px 8px", borderRadius: 4, fontSize: 11, fontWeight: 600 }}>Admin</span>
                  ) : (
                    <span style={{ background: "rgba(255,255,255,0.08)", color: C.textMuted, padding: "2px 8px", borderRadius: 4, fontSize: 11 }}>-</span>
                  )}
                </div>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
                  <span style={{ width: 6, height: 6, borderRadius: "50%", background: u.is_active ? C.green : C.danger }} />
                  <Text size="xs" style={{ color: u.is_active ? C.green : C.danger }}>{u.is_active ? "Active" : "Inactive"}</Text>
                </div>
                <div style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}>
                  <Button onClick={() => handleToggle(u.id, "is_superuser", u.is_superuser)} size="compact-xs" variant="light" disabled={!!toggling[u.id + "/is_superuser"]}>
                    {!!toggling[u.id + "/is_superuser"] ? "..." : u.is_superuser ? "Revoke Admin" : "Make Admin"}
                  </Button>
                  <Button onClick={() => handleToggle(u.id, "is_active", u.is_active)} size="compact-xs" variant="light" disabled={!!toggling[u.id + "/is_active"]}>
                    {!!toggling[u.id + "/is_active"] ? "..." : u.is_active ? "Deactivate" : "Activate"}
                  </Button>
                </div>
              </div>
            ))}
