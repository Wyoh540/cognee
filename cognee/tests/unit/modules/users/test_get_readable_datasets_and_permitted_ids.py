...
@pytest.mark.asyncio
async def test_get_readable_datasets_preserves_request_scoped_user(monkeypatch):
    request_user = object()
    received = []

    async def fake_get_specific(user, _permission_type, _dataset_ids):
        received.append(user)
        return []

    monkeypatch.setattr(readable_module, "get_specific_user_permission_datasets", fake_get_specific)

    await readable_module.get_readable_datasets(request_user)

    assert received == [request_user]
...
@pytest.mark.asyncio
async def test_get_permitted_dataset_ids_preserves_request_scoped_user(monkeypatch):
    request_user = object()
    received = []

    async def fake_get_readable_datasets(user):
        received.append(user)
        return []

    monkeypatch.setattr(permitted_ids_module, "get_readable_datasets", fake_get_readable_datasets)

    await permitted_ids_module.get_permitted_dataset_ids(request_user)
