...
def _patch_engine(monkeypatch, module, session):
    ...
    async def fake_has_membership(*_args, **_kwargs):
        return True

    monkeypatch.setattr(module, "has_tenant_membership", fake_has_membership)


def _deny_membership(monkeypatch, module):
    async def fake_has_membership(*_args, **_kwargs):
...
@pytest.mark.asyncio
async def test_tenant_outsider_cannot_list_roles(monkeypatch):
    """A request cannot inspect roles outside its selected workspace."""
    tenant_id = uuid4()
    requester = FakeUser(uuid4())
    session = FakeSession([])

    _patch_engine(monkeypatch, _roles_mod, session)
    _deny_membership(monkeypatch, _roles_mod)

    with pytest.raises(PermissionDeniedError):
        await get_tenant_roles(tenant_id=tenant_id, user=requester)
