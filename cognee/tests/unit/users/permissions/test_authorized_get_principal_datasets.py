...
@pytest.mark.asyncio
async def test_request_scoped_user_keeps_selected_tenant(monkeypatch):
    requester_id, selected_tenant = uuid4(), uuid4()
    requester = FakeUser(requester_id, selected_tenant)
    mine, theirs = FakeDataset(selected_tenant), FakeDataset(uuid4())
    _patch(
        monkeypatch,
        uuid4(),
        FakePrincipal("user", principal_id=requester_id),
    ...
    async def fail_if_refetched(_user_id):
        raise AssertionError("request-scoped user must not be refetched")

    monkeypatch.setattr(_mod, "get_user", fail_if_refetched)

    datasets = await authorized_get_principal_datasets(requester_id, "read", requester)
