...
async def authorized_get_principal_datasets(
    principal_id: UUID, permission_name: str, requester: Union[User, UUID]
) -> List[Dataset]:
    """
    ...
    Args:
        ...
        requester: Request-scoped user, or its ID for backwards compatibility.
    ...
    """
    if isinstance(requester, UUID):
        requester = await get_user(requester)
