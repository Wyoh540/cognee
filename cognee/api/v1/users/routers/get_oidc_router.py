    ...
    async def oidc_login(slug: str, request: Request):
        ...
        async with engine.get_async_session() as session:
            ...
            response = RedirectResponse(
                authorization_url(document, provider, redirect_uri, state, challenge)
            )
            response.set_cookie(
                STATE_COOKIE,
                state_token,
    ...
    async def oidc_callback(slug: str, request: Request, code: str, state: str):
        ...
        async with engine.get_async_session() as session:
            provider = (
                await session.execute(
                    select(OIDCProvider).where(
                        OIDCProvider.id == UUID(state_data["provider_id"]),
            ...
            tokens = await exchange_code(
                document, provider, code, redirect_uri, state_data["verifier"]
            )
            if not tokens.get("id_token"):
                raise HTTPException(400, "OIDC provider did not return an ID token.")
            claims = await validate_id_token(
                document, provider, tokens["id_token"], state_data["nonce"]
            )
            ...
            if not user:
                ...
                if not user:
                    user = await create_user(
                        email=email.lower(), password=secrets.token_urlsafe(48), is_verified=True
                    )
                session.add(
                    OIDCIdentity(provider_id=provider.id, user_id=user.id, subject=claims["sub"])
                )
                try:
            ...
            response = RedirectResponse(
                os.getenv("OIDC_FRONTEND_REDIRECT_URL", "http://localhost:3000/")
            )
            response.set_cookie(
...
def get_oidc_admin_router() -> APIRouter:
    ...
    async def list_providers(user: User = Depends(get_authenticated_user)):
        ...
        async with engine.get_async_session() as session:
            providers = (
                (await session.execute(select(OIDCProvider).order_by(OIDCProvider.name)))
                .scalars()
                .all()
            )
    ...
    async def create_provider(body: ProviderBody, user: User = Depends(get_authenticated_user)):
        ...
        provider = OIDCProvider(
            ...
            client_secret_ciphertext=encrypted[0],
            client_secret_nonce=encrypted[1],
    ...
    async def update_provider(
        provider_id: UUID, body: ProviderBody, user: User = Depends(get_authenticated_user)
    ):
        _require_superuser(user)
        issuer = validate_issuer(body.issuer)
        await discovery(issuer)
        ...
        async with engine.get_async_session() as session:
            ...
            if not provider:
                raise HTTPException(404, "OIDC provider not found.")
            provider.name, provider.slug, provider.issuer = (
                body.name,
                normalize_slug(body.slug),
                issuer,
            )
            provider.client_id, provider.scopes, provider.enabled = (
                body.client_id,
                body.scopes,
                body.enabled,
            )
