...
def get_permissions_router() -> APIRouter:
    ...
    async def get_principal_datasets(
        ...
        datasets = await method_authorized_get_principal_datasets(
            principal_id, permission_name, user
        )
    ...
    async def create_tenant(tenant_name: str, user: User = Depends(get_authenticated_user)):
        ...
        Only super admins can create tenants (workspaces).
        ...
        ## Error Codes
        - **403 Forbidden**: Only super admins can create tenants
        ...
        if not user.is_superuser:
            return JSONResponse(
                status_code=403,
                content={"detail": "Only super admins can create new workspaces."},
            )
    ...
    async def get_tenant_roles(
        ...
        ## Error Codes
        - **403 Forbidden**: Caller is not a member of this tenant
        """
    ...
    async def get_users_in_role(
        ...
        ## Error Codes
        - **403 Forbidden**: Caller is not a tenant member, or is not a member of the role and lacks user-management permission
    ...
    async def get_user_roles(
        ...
        Any member of the tenant can view role assignments.
    ...
    async def get_users_in_tenant(
        ...
        Any member of the tenant can view the member list.
